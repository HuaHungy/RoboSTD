...
class LerobotSimReplayer:
    ...
    def get_frame_data(self):
        ...
        for key in keys_to_load:
            if key in df.columns:
                raw_names = info["features"][key]["names"]
                # 兼容 name 是单个列表，或者二维列表的情况
                names = raw_names[0].copy() if len(raw_names) > 0 and isinstance(raw_names[0], list) else raw_names.copy()
                ...
                if gripper_key in info["features"] and gripper_key in df.columns:
                    # 合并名称
                    gripper_raw_names = info["features"][gripper_key]["names"]
                    gripper_names = gripper_raw_names[0].copy() if len(gripper_raw_names) > 0 and isinstance(gripper_raw_names[0], list) else gripper_raw_names.copy()
    ...
    def _update_plot(self):
        try:
            ...
            for k in self.all_plot_keys:
                # 优先从原始数据帧（raw_data_frame）中获取值，以显示原始关节值而不是归一化或修正后的值
                # 如果原始数据中没有（例如是 sim 仿真生成的位姿数据），则从当前 data_frame 中获取
                val = self.raw_data_frame.get(k, self.data_frame.get(k, 0))
    ...
    def step(self) -> bool:
        try:
            ...
            # 保存一份原始数据（未经过 fix_method 或 gripper 映射）用于绘图显示原始值
